...
RSpec.describe YnabPayeeSyncJob, type: :job do
  describe '#perform' do
    let(:plan_id) { 'fake-plan-id' }
    let(:mock_client) { instance_double(YNAB::API) }
    let(:mock_api) { instance_double(YNAB::PayeesApi) }
    ...
    before do
      ...
      allow(mock_client).to receive(:payees).and_return(mock_api)
    end
    ...
    context 'when no previous server knowledge exists' do
      it 'fetches all records, saves them, and saves the new server knowledge within a transaction' do
        mock_response = double('response', data: double('data', server_knowledge: 100, payees: []))
        expect(mock_api).to receive(:get_payees).with(plan_id).and_return(mock_response)

        expect(ActiveRecord::Base).to receive(:transaction).and_call_original

        described_class.new.perform(plan_id)

        knowledge = ServerKnowledge.find_by(plan_id: plan_id, topic: 'payees')
    ...
    context 'when previous server knowledge exists' do
      before do
        create(:server_knowledge, plan_id: plan_id, topic: 'payees', knowledge: 50)
      end

      it 'makes a delta request passing the previous server knowledge' do
        mock_response = double('response', data: double('data', server_knowledge: 105, payees: []))
        expect(mock_api).to receive(:get_payees)
    ...
    context 'when an error occurs' do
      it 'logs the error and re-raises it, aborting the transaction' do
        expect(mock_api).to receive(:get_payees).and_raise(StandardError.new('API Failure'))
        expect(Rails.logger).to receive(:error).with(/YnabPayeeSyncJob failed for plan_id fake-plan-id: API Failure/)
    ...
    context 'processing delta payloads' do
      let(:response_data) { double('data', server_knowledge: 100, payees: items) }
      let(:mock_response) { double('response', data: response_data) }

      before do
        allow(mock_api).to receive(:get_payees).and_return(mock_response)
      end
      ...
      context 'with a deleted record' do
        ...
        it 'destroys the local record' do
          ...
          expect(Payee.find_by(ynab_id: 'record-2')).to be_nil
        end
      end
    end
  end
